...
### 走行までの流れ
1. ラズパイとモバイルバッテリーを繋ぎ電源を入れる
2. コントローラの電源を入れ点灯状態になったことを確認する
3. PCでラズパイにリモート接続する
4. ラズパイ内でターミナルを立ち上げる
5. `(env) pi@raspberrypi:~/togikaidrive83 $` と表示されていることを確認する
6. 車体の動力電源スイッチをONにする
7. `python3 run_spatial.py` と入力しエンターキーを押す
8. 超音波センサーの異常チェックに問題ないことを確認する
9. カメラ画像が立ち上がり、ターミナルに`Enterを押して走行開始!`の表示が出たことを確認しエンターキーを押す ※ターミナル画面をクリックしアクティブ状態にすること
10. コントローラ操作の場合、右上のΞボタンを押し走行モードをマニュアルモードに切り替える

### 停止させる
ターミナル画面をアクティブ状態にしてCtrl＋Cを押す

### Pythonコード一覧
```
# 超音波＋画像認識で走行
python3 run_spatial.py     # 超音波＋画像認識で走行
  
# 走行確認用
python3 run.py             # 超音波センサーのデータ取り、確認に使用

# 認識確認用
python3 yolo.py            # コード内のmodelとjsonファイルパスを適宜書き換え
python3 yolo.py -s 2       # 画像撮影用　数値はFPS(枚/秒)
